#!/usr/bin/env python
"""
Script simplificado para poblar el módulo financiero con datos de prueba
Módulo 2: Gestión Financiera Básica - T2: Consultar estado de cuenta

Ejecutar:
python manage.py shell
exec(open('scripts/poblado_db/poblar_modulo_financiero_simple.py').read())
"""
...
def poblar_cargos_residentes():
    ...
    # Obtener residentes (buscar usuarios que no sean admin/security)
    residentes = User.objects.exclude(username__in=['admin', 'security']).exclude(is_superuser=True)[:5]
...
def mostrar_resumen():
    ...
    print(f"📋 Conceptos financieros: {total_conceptos} (Activos: {conceptos_activos})")
...
if __name__ == "__main__":
    ...
    # Si se ejecuta desde shell, llamar main directamente
    main()